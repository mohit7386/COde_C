...
void helloWorld(int n){
    ...
    helloWorld(n-1); //here function calls itself and we are writing (n-1) because if we do not write this thing then our recursive function will go in infinite loop..Kyuki agar hum (n-1) nahi likhenge to jo value minus hoke aayegi wo to check hogi hi nahi ar check nhi hogi to har baar true condition milegi to wo har baar print karata rahega..aur jab (n-1) agar hum de diye to ye check karega condition aur zero milne ke baad ruk jaayega... 
}