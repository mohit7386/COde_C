...
int main(){
    ...
    printf("%d\n",abc); //here we do not print the value of the a we are printing the address integer value of the abc that's why some value is printed 
    printf("%d\n",&_age);
    printf("%d",_age);
    return 0;
    //In pointer asterix (*) means value at that address..
    //ptr represents the memory address
    //*ptr represents the value at the memory address
}    