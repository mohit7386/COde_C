//Write the function to count the occurences of vowels in a String...
#include<stdio.h>
#include<string.h>
...
int countVowels(char str[]){
    ...
    for(int i=0;str[i]!='\0';i++){
        if(str[i]=='a' || str[i]=='e' || str[i]=='i' || str[i]=='o' || str[i]=='u' ){ //Comparing with each element of the array and when match successful then the value of count will increase...
            count++;
            printf("%c",str[i]); //When we give the \n here then ye ek print karne ke baad line change karega baaki ek ko upr print karega ar baaki ke ko neeche print karega 
        }
    }
    printf("\n");
    return count;
